...
| Whose money a screen is reading | The readers' own signatures | Every household-scoped reader takes the **owner filter** as a required first argument with no default; the account-scoped ones do not take it at all, because an account already has exactly one owner (ADR-0008). `manualNetWorth` and `latestObservedSession` are out for their own reasons, stated at the seam. A required argument cannot make honouring the filter impossible to skip — a new screen can pass `ALL_OWNERS` and draw no control — but it makes the skip **visible in review** rather than invisible by omission, which is the most a signature can do. The same trick, for the same reason, as the chart's required `masked` prop. |
