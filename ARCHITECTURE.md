...
| Settling the owner-filter reading — the address, the roster, `reading` | `app/lib/owner-reading.server.ts` | The four owner-filter screens call `ownerReading` once, first, for the settled address and for `reading` — what a household-scoped reader on that screen narrows by. The module does not call the household-scoped readers itself: the obligation is on the callers, which still take the owner filter as a required, undefaulted first argument by hand (the "Whose money a screen is reading" row below) and pass `reading` to it — or, for the chart's three reads, to `chart-series.server.ts` (spec 0015) as a required field of the `ChartScope` it hands that module in the reader's place — so whose money a loader reads stays visible in review rather than hidden inside this one. |

**Shared primitives, with exceptions that are documented rather than denied.**

| Invariant | The primitive | The exceptions |
|---|---|---|
| Money representation and its rounding | `app/lib/money.ts` | Several modules do `BigInt` arithmetic on `money.ts`'s units, which is the intent. What is meant to exist once is the *rounding rule*, and it is spelled twice: `positions.server.ts:276` rounds the overflow-guard product inline instead of calling `divide`. |
...
| Whose money a screen is reading | The readers' own signatures — the **owner filter** is a required first argument with no default on every household-scoped read (ADR-0008, and §6.3 on where the narrowing then goes), except the chart's three (`firstRecordedDate`, `netWorthSeries`, `netWorthSessionSeries`): a screen never calls those directly, and instead names `reading` as a required field of the `ChartScope` it hands `chart-series.server.ts` (spec 0015) — still refused without it, so the skip is exactly as visible in review as a missing positional argument would be | The account-scoped readers, which do not take it because an account already has exactly one owner, and `manualNetWorth` and `latestObservedSession`, which are out for their own reasons — both given in `manualNetWorth`'s own docstring, where the line falls. The argument cannot make the filter impossible to skip — a new screen can pass `ALL_OWNERS` and draw no control — only **visible in review** rather than invisible by omission, which is the most a signature can do. The same trick, for the same reason, as the chart's required `masked` prop. |
...
before any database work happens (`holdings.tsx:144`), so `?group=account&owner=2` would 302 first.
...
2. **Filtering and grouping are pure functions over one array**, not seven new SQL predicates. The
   ...
   an open `?edit=` row costs exactly one more read (`holdings.tsx:257`), for the date the correction
   will be filed under.
...
renames a person and `:248` deletes one outright when they own no accounts, `settings.server.ts:88`
...
```
   ...
   Overview      netWorth, manualNetWorth,        account totals, chart series
                 netWorthChange, chartSeries ──▶ chart-series.server.ts
   ...
   Account       accountTotal, accountHoldings,
                 uploadReceipt, chartSeries ──▶ chart-series.server.ts
```
...
| `chart-series.server.ts` | The chart's own reads (spec 0015): `chartAnchors(scope)` reads a surface's earliest recorded date and the latest observed session together; `chartSeries(scope, resolved)` picks the reader the resolved window implies and applies §6.3's `coverage.total > 0` rule once, for both chart surfaces, instead of in each route as before. `ChartScope` carries the household reads' owner filter as a required field, `reading`, rather than as a positional argument — still refused without it, so the narrowing stays visible in review (ADR-0008) |
