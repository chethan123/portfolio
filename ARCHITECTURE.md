...
guard arms itself on the first upload, and every later statement is checked against it; a settings
form open across that capture cannot disarm it, because the save compares the box against the number
it was drawn with (§7.2). Zero rows written refuses (a stored blank): a blank is not none to the
guard's `is null` predicate, so `recordAccountNumber` (`uploads.server.ts:2096`) treats it as a case
Settings must clear first rather than write over silently.
