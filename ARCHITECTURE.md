...
| `price-provider.server.ts` | The provider interface, all three methods, and the symbol probe. The methods include the raw entry a quote hands on for the archive, attached past every refusal, the split un-adjust a history goes through, and the anniversary-rule sum a trailing dividend goes through. The library itself is reached through `server/yahoo-client.ts`, its only importer |
