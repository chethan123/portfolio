...
│                    instrument-resolution · prices · current-statement · …    │
...
2. **Filtering and grouping are pure functions over one array**, not seven new SQL predicates. The
   ...
   an open `?edit=` row costs exactly one more read (`holdings.tsx:148`), for the date the correction
   will be filed under.
...
CRUD should: `accounts.server.ts`'s `updateAccount` edits an account and `closeAccount` closes one,
`people.server.ts:117` renames a person and `:138` deletes one outright when they own no accounts,
...
are not the same slice of anything (argued at `allocation.ts:12-13`, applied in `allocateShares` at
...
The single most consequential line of infrastructure in the codebase is in `server/db.ts:11-15`:
...
- Postgres reports every column of a view as nullable regardless of the underlying column, so
  `holding_valued` comes out entirely `| null`. That is why `required()` in `valuation.server.ts`
  exists: a narrower that throws, wrapped in `toValuedHolding` around every column the underlying
  tables guarantee. The view is typed *from* a table, not typed *like* one.
...
- **The review revision binds the server-rendered interpretation.** `assembleDiff` hashes the raw
  ...
  keeps the stale-review warning. The comparison and its order — rerouted, then the revision, the
  reviewed-date reproduction deciding date-changed over stale, then per section the baseline — are
  `verifyBinding` in `review-form.ts`, pure, called once under the locks; `commitUnderLocks` only
  re-assembles at the date `dateToReproduce` names.
- **The baseline binds the confirmation to what it was drawn against (#181).** `assembleDiff`
  ...
  every reason that diff disagrees with what the form still believes — a posted `baselineSetId` the
  fresh diff no longer matches (`verifyBinding`'s `voided`, in `review-form.ts`, decides this), an
  unconfirmed filed-behind statement, or an unconfirmed majority removal — and throws once, naming
  every applicable one. Revision mismatches are classified first even when that baseline moved, so a
  baseline refusal cannot conceal another statement or account change. When the current revision
  matches, the stale-baseline sentence fires whenever the baseline moved and no unconfirmed
  filed-behind demand is left to subsume it — reason 2 subsumes reason 1 whenever it applies, not
  only on an untouched first submission. Ticks are irrelevant to it. An undated file's first render
  is already classified against today; when the reader chooses another date, **Review this date**
  draws and binds that date's baseline before a commit can use it (see §7.2's baseline-moving row).
...
statement of the rule. The guard arms itself on the first upload, and every later statement is
checked against it; a settings form open across that capture cannot disarm it, because the save
compares the box against the number it was drawn with (§7.2). Zero rows written refuses (a stored
blank): a blank is not none to the guard's `is null` predicate, so `recordAccountNumber`
(`uploads.server.ts`) treats it as a case Settings must clear first rather than write over silently.
...
number. `routeStatement` also owns which wizard step each routing problem belongs to.
...
itself is `withRefreshLock` in `prices.server.ts`):
...
```
   ...
   ─────────────────────────────────────────────────────────────────────────────
   Overview      netWorth, manualNetWorth,        account totals, chart series
                 netWorthChange, and the chart's
                 own reads through
   ...
   Account       accountTotal, accountHoldings,
                 uploadReceipt, and the chart's
                 own reads through
                 chart-series.server.ts
```
...
synthesised if the selected key matches nothing (`holdings-view.ts:334`). A stale bookmark from before an account
...
| Two upload commits over the same account(s), at least one of them several accounts | Both lock every account they touch through `withAccountLock`/`withAccountLocks`, ascending `compareIds` order, so the second queues on whichever shared account the first reached first rather than deadlocking. Once the first commits, the second's own re-read under its locks — the re-parsed draft, the re-routed groups, the re-resolved diff — no longer matches what it posted: a changed review revision refuses it outright, and history the first wrote that leaves the diff's own shape unchanged still moves the affected account's append watermark, which the second's per-account check (decision 9) refuses, naming that account | `accounts.server.ts` (`withAccountLock`, `withAccountLocks`), `uploads.server.ts` (`commitUnderLocks`), `review-form.ts` (`verifyBinding`) |
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  withDatabase(body) — everything that touches data                       │
...
┌──────────────────────────────────────────────────────────────────────────┐
│  Direct Postgres, no rollback wrapper                                    │
...
| `uploads.server.ts` | Drafts, multipart reading, the dated diff and its review revision, and `recordUpload`, the ingest flow's one write: `commitUnderLocks` for an account chosen up front or a file of several, routed by `statement-routing.server.ts`. Each transaction enters through `withAccountLock`, or nested per routed account through `withAccountLocks`, locks the draft, then promotes, records the account number and verifies aliases before appending history (§6.1, §7.2). `readDraft` is the one read a wizard request takes its result from — the file and the step. Also `recordedStatements`, the done page's own reader, off `position_set` ids rather than the commit |
...
| `interpreted-number-input.tsx` | The progressive form control for the shared decimal rules: the fixed interpretation rule is in server markup, then a masking-aware punctuation-normalization echo follows generic-valid input after hydration without promising a contextual write will succeed. A server refusal stays visible, invalid and described only while the box still carries its returned value; editing makes the current client state authoritative, and a new failed submit activates its new refusal |
...
The four modules every test is written through. Why the suite is shaped this way is §9; this is
