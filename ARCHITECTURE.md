...
| `decimal-input.ts` | The browser-safe exact-string grammar shared by every typed financial field and its live interpretation: commas and spaces group thousands only in threes, a point alone marks decimals, and the legacy leading `+` and trailing point remain valid |
