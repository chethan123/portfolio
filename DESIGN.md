...
pick account from dropdown
  ...
  → diff preview          (added · updated · removed — every removal listed in full)
...
- **Undo stays free.** A wrong or partial file never overwrote anything, so removing its set
  restores the prior truth intact. No screen deletes a set yet (§5.4 records the gap); today that
  removal is an operator's `psql` statement, and
  [`docs/importing-history.md`](docs/importing-history.md) shows it.
...
A blank instrument row is harmless only when its mapped quantity and cost-basis cells state no
figure. Zero or an invalid figure makes the row a blocking mapping problem; the shared absence
spellings remain harmless. The Columns screen names its source line and populated columns before a
mapping can be saved; a draft saved under an older rule shows the same block on Review and offers no
commit. A mapping correction returns to Columns; a source-file correction starts a new upload
because the draft retains its original bytes.
