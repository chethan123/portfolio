/**
 * Seeds the domain vocabulary (DESIGN.md §4) directly, so a schema change touches only this file.
 * Money/quantity cross the driver boundary as decimal strings.
 */
...
export type Fixtures = {
  ...
  /** One backfill attempt (ADR-0011); count/error tied to `outcome` by the ledger's check constraints. */
...
/** Bootstrap race's three statements, over a raw pg handle — one Kysely txn can't
 * exercise two connections racing on `passkey_bootstrap_idx`. */
