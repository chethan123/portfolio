...
describe("headerRowChoices", () => {
  ...
  it("keeps blank rows out and the row on screen in, whatever detection thinks", () => {
    ...
    // Blank spacer never offered; candidates already cover the rest, nothing appended.
    expect(headerRowChoices(rows, 0)).toEqual([0, 2]);
    // Requested row outside the list (stale ?header=) still renders as chosen, not vanished.
