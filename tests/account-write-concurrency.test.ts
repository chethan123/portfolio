...
import { closeTestDatabase, testDatabase } from "./support/database.ts";
import {
...
describe("account write serialization", () => {
  it("preserves two accepted corrections that overlap on different holdings", async () => {
    ...
    // Begins first: with the old now() default, its later insert would sort behind the lock holder's.
