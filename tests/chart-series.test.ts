...
/** Dated window naming exact dates. chartSeries reads only dates/session off RangeWindow —
 * range/since are filler no test here needs to vary. */
...
describe("the coverage rule, at the assembly seam rather than only inside the reader beneath it", () => {
  it(
    ...
    withDatabase(async ({ seedPerson, seedAccount, seedPositionSet, usdInstrument }) => {
      ...
      // 2026-01-10 predates the first statement — no rows, not zero (§6.3) — only
      // 2026-01-15 survives, on both scopes.
