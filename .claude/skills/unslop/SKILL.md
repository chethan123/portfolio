---
name: unslop
description: Cut AI tells from prose.
argument-hint: "[file, the diff, or nothing for the last reply] [extra rules for this pass]"
...
The arguments say what to edit and may add rules for this pass ("no emdashes", "tighten it"):

> $ARGUMENTS

Resolve them before scanning:

- A path or glob: edit those files in place with the Edit tool.
- "the diff", "the changes", "the new docs": the prose this branch added or changed. `git diff` against the base branch decides which lines those are. Leave the untouched lines alone unless told to clean the whole file.
- Empty, "that", or "this": the last thing you wrote in this conversation. The reply is the cleaned text in full, nothing else.
- Text pasted or quoted in the prompt: return the cleaned text.

Prose only. Code, comments in code, commands, identifiers, paths, quoted text, error output, and anything in backticks or a fenced block stay as they are, even where a rule below would fire on them. Code comments follow `AGENTS.md`, not this skill.
...
4. When the target was a file, reply with one line per file naming the rules that fired. The diff is the record.
