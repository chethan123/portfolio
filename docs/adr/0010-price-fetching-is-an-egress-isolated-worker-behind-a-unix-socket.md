...
talks to the internet and the largest share of that tree by itself. It also connected to Postgres
...
compromised together, one image and one npm tree serving both ends of whatever crosses between them,
...
disqualifies that shape rather than losing to a better trade. A refresh is now quotes and a backfill
batch whose candidate selection joins `holding` and `position_set`, exactly the tables a price-only
worker must never read. A worker that owns the refresh therefore needs those tables, or a
...
- **A unix socket in a tmpfs volume the two containers share.** The named volume `price-worker-sock`
  ...
  and nothing else on it. The volume is therefore its only link to the rest of the stack, and that
  network is its only way out. Two links, each with one thing on the far side, is the whole topology.
...
- **The worker holds no database credential and no TCP listener.** No `pg`, no Kysely, nothing under
  ...
  two things this design has to guarantee hold by construction rather than by a rule to keep: no
  route from the worker to the database, and no route from the worker to the app's own screens.
...
- availability hardening the login demanded of the whole cluster, which meant `REVOKE`s on the
  advisory-lock and large-object families, `TEMP` revoked from `PUBLIC` and `temp_file_limit`, plus
  a second test running the worker's statements under `SET LOCAL ROLE`;
...
the mailbox had one: **`LISTEN`/`NOTIFY`**, since `pg` has no reconnect logic for a dropped `LISTEN`
and a notification is unqueued, so a poll was still needed regardless of it; **RLS**, to keep
...
- **No new UI state.** The freshness component and the refresh route's outcomes are untouched. The
  dead-worker distinction between a connect failure, Yahoo failing and a slow answer is the
...
- **One image, entrypoints either side of which restart independently.** `worker` and `egress-proxy`
  are two more entrypoints on `app`'s own image, three services from one build, with `dump` a
  Postgres image and no relation. The socket plus the library's raw JSON is the whole contract
  between them: an `up -d` that lands one side of a version bump before the other is harmless
  exactly because nothing but that contract crosses.
