...
([`docs/specs/0002-pricing.md`](../specs/0002-pricing.md), Out of Scope). DESIGN.md §7's
...
**Superseded in part** by [0018](../specs/0018-price-worker.md). "spec 0015" here, in the paragraph
above, under "In-process", under "Mailbox-shaped, for the worker" and in "The outbound surface
grows", is the price-worker proposal that was never indexed and is deleted with 0018; by
...
case and by years for a household loading its history. `holding_valued_at(d)` then finds no close at
...
- **Ledgered.** One `price_backfill` row per attempt per instrument, recording when, the range
  asked for, how many closes were new, and a closed outcome. An unfillable gap is therefore retried
  daily rather than every tick, and "why is this still unpriced in March" is a query rather than a
  memory.
...
- **A manual "backfill N years" screen.** Rejected: it asks the household to know that a spine
  exists, that it has a head, and how far back a statement reaches. The answer to all three is
