...
The projected annual dividend is a column on the `holding_valued` view, and `holding_valued_at`
reports it as null. Both objects were replaced in one migration, and they have to be. Appending a
column to the view succeeds silently while leaving the function broken, and nothing fails until
something calls it. This record exists for whoever adds the next column, because the tooling will
not stop them.
...
- **The null is the point, not an oversight.** There is no historical dividend anywhere, because
