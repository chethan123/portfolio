...
`account.tsx`), leaving one usable point, which is one point short of a line. The value moves every
day only from price refreshes, so the missing line was a sampling artifact, not a data gap.

We changed `sampleWindow` to sample every calendar day when the whole span fits inside a fixed
budget (180 samples). Once a span exceeds that budget, it spaces samples with gaps that grow
geometrically walking backward from the window's own end (`until`) rather than from literal "today".
A custom range with a past end date decays away from its own boundary. The gap nearest the anchor is
fixed at exactly one calendar day, and a numeric solve sets the growth ratio so the accumulated
gaps land exactly on the window's start. This guarantees the day immediately before the anchor is
always sampled, for every preset, without treating recency as a special case.
...
re-tuning if the budget or the presets change. That is the same magic-constant problem a smooth
curve was chosen to avoid.
...
same date as the anchor itself. That silently breaks the "always a day back" guarantee for the
exact spans the redesign exists to fix.
...
- The output stays the same shape, an ordered list of ISO dates, so every other consumer of the
  resolved date list is unaffected: `netWorthSeries`, `NetWorthChart`, `overview.tsx` and
  `account.tsx`.
