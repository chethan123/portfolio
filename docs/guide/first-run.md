...
- **No sign-out control.** Closing the tab does not sign you out. If a phone is lost, and the
  household has enrolled passkeys, revoke that phone's passkey from Settings → Passkeys on any
  browser that is still unlocked. See [Removing a passkey](passkeys.md#removing-a-passkey). Any
  browser that got in with that passkey is locked again straight away; one that was let in by
  approving on a different device is locked by the clock instead, within fifteen minutes. Whoever
  runs the instance is the recovery only when no passkey can be reached at all, and stopping an
  address getting in through the front door is still theirs to do, from outside the app.
...
The prompt is not dismissible, and it is hidden while you are inside Settings, because it would be
...
beneath it. The filled **Upload statement** button below that is how a statement gets in.
...
in the household has enrolled a passkey, which is done at Settings → Passkeys, every browser is
refused every screen until it proves itself with its own passkey check. That holds however recently
the browser signed in through Google. Signing in again does not clear it, and unlocking one browser
does nothing for any other.

A brand-new instance holds no passkey at all, so none of this applies yet. The moment somebody
enrols the first one, though, every *other* browser in the household is locked from the very next
thing each of them asks the server for, not just the one doing the enrolling. See
...
stores nothing on the phone. When the app cannot reach home, off the VPN or with the instance
