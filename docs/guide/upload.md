...
- **No OFX, QIF, XLSX or PDF.** The first screen checks one thing: that the file reads as text. A
  spreadsheet or a PDF is binary in practice and is refused there. Anything that does read as text,
  such as OFX, QIF or the odd all-text PDF, gets past that check and usually stops at the mapping
...
When an institution offers no CSV download, as with a pension portal or a paper statement, copy
...
where a cost basis is unknown (write `n/a`, never `0`, because a zero basis reports a fake gain).
...
**"lists what is owed as a positive number"** is a checkbox, captioned with the account's name. A
...
- **Point it at something already listed.** Nothing new is created; the spelling is attached.
...
**Price source** is **Feed**, looked up automatically and needing a symbol, or **Manual price**,
