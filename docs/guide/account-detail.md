...
The caption above the line reads exactly as it does on Overview: the point the line ends at until
you point at the chart, then whichever point is nearest. Nothing here is ever captioned as
hand-typed, because the chart never draws the household's hand-typed pre-history. That figure is
the household's net worth, not this account's.
