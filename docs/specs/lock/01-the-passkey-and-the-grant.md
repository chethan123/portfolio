...
- [ ] Only one passkey may be the household's first, and it takes **two** mechanisms rather than
      ...
      rule requires. What the pair guarantees is those two interleavings and no third: it does not
      serialise a bootstrap insert against an *ordinary* one whose own snapshot still saw the passkey
      that authorised it, which is one statement wide under autocommit and is stated rather than
      closed (spec 0020, ticket 09; migration 0012's own comment carries the argument)
