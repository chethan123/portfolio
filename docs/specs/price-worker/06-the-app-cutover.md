...
[03](03-the-two-hardening-rules.md), [05](05-deploy-the-worker-alongside.md).
...
- [ ] `ask(kind, body, { budgetMs } = …)` with the production budgets as defaults, so a test passes
      ...
      "application/json" }, signal: AbortSignal.timeout(budgetMs) })`, the body `JSON.stringify(body)`;
      the response read to 2 MiB and the request destroyed past it (a ten-year chart answer is
      around 300 KB); `JSON.parse` of the whole. No handle, no flag: a dead worker costs one connect
      failure per call site, and the batch abort of [01](01-one-refresh-and-the-batch-abort.md) is
      what keeps a tick's cost at two of them (spec §3.3)
- [ ] The outcomes, told apart in this order: a request `error` whose `code` is `ENOENT` or
      `ECONNREFUSED` → `ProviderUnreachable` with the message `no worker listening at <path>`; the
      signal's `TimeoutError` → an `Error` saying the worker did not answer within the budget; the
      body cap → an `Error` naming it; a status other than `200` → an `Error` carrying the body's
      `error` text, or the status when the body has none; `200` → the parsed body. Budgets as
      constants with their reasons: `quotes` 15 s, `history` 30 s (the worker's watchdog), `probe`
      10 s (a cold worker's first probe pays a three-fetch crumb handshake, and the verdict a short
      budget loses is `non-usd`, the one a person acts on)
...
- [ ] `socketProbe: ProbeSymbols` — one `ask("quotes", …)`, then `probeVerdicts`
...
- [ ] The module header carries the argument: why a unix socket and not a TCP port on an internal
      ...
      socket (the worker's watchdog is its own, spec §3.5)
...
- [ ] `docs/operating.md:761`'s four causes gain the fifth — the worker is not listening: dead,
      restarting, or absent because `compose.yaml` was never replaced and the volume does not exist.
      The stem is still `Price provider failed`, the text says "no worker listening at
      /run/price-worker/worker.sock", and `docker compose ps` shows `worker` unhealthy, restarting
      or missing. The upgrade note repeats [05](05-deploy-the-worker-alongside.md)'s compose-file
      rule and its symptom. The full record is [09](09-documents-and-runbooks.md)'s
...
- [ ] `tests/provider-socket.test.ts` (new), no database for the transport cases:
      `process.env.PRICE_WORKER_SOCKET` set to a path under `os.tmpdir()` **before** any import that
      reaches `getConfig()` (`tests/price-poller.test.ts:37` is the precedent, for `DATABASE_URL`;
      `getConfig` memoises, `server/config.ts:150-153`); a real `startWorker`
      ([04](04-the-price-worker-process.md)) on that path with a fake Yahoo client per case, closed
      in `afterEach`. Cases: `getQuotes` returns the parsed quotes and skips a `CurrencyRefused`;
      `getDailyCloses` sends the `matchKey`'d symbol and `range.from` and applies `until`; a `502`
      saying "No data found" is `no-history`; a `502` with other text throws it; no server on the
      path throws `ProviderUnreachable` naming the path, and within a second — the assertion that
      it is a connect failure and not a grace; a fake whose `quote` never resolves under `budgetMs:
      200` throws the budget error; a `429` throws; a `200` body over 2 MiB throws; a symbol with a
      slash is dropped and logged and the request carries the rest; 101 symbols are two requests
