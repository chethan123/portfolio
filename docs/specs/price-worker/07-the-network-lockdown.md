...
- [x] The networks exactly as spec §3.6: `backend`, `caddy-app` and `caddy-gate` internal with
      ...
      upgrade note names
- [x] **`compose.external-db.yaml`**, shipped by this ticket, because this is the release that would
...
- [x] The header repeats the Engine 28.0 floor [05](05-deploy-the-worker-alongside.md) declared, now
      load-bearing,
...
- [x] Caddy's reachability walk still holds against the `Caddyfile`: `/healthz` and the catch-all to
...
- [x] `db` (`:59`): `POSTGRES_PASSWORD: "${POSTGRES_PASSWORD:?missing POSTGRES_PASSWORD — see
      docs/operating.md, Upgrading}"`
- [x] `app` (`:204`) and `dump` (`:126`): the `DATABASE_URL` default becomes
...
- [x] `.env.example:23`'s explicit `DATABASE_URL` line goes; the comment says when to set one (your
...
- [x] The header's "every other setting has a working default" (`:20`) is rewritten:
      `POSTGRES_PASSWORD` joins the settings deliberately without one
- [x] The lines that still say the password lives in a URL, each rewritten for `POSTGRES_PASSWORD`:
...
- [x] A numbered sequence with each step's reason, in the only order that runs — the new
...
- [x] The rollback note: rolling `APP_VERSION` back under this compose file starts an image that
...
- [x] `:308-319`'s rotation recipe and `docs/runbook.md:525-552` lose the URL half: `.env` first,
      then the role, then `up -d`

**Smoke** (`scripts/smoke-test.sh`)

- [x] Export a throwaway `POSTGRES_PASSWORD` before the refusal check (`:108-116`) and add the
...
- [x] Under `compose.external-db.yaml` (`COMPOSE_FILE=compose.yaml:compose.external-db.yaml`, no
...
- [x] From `app` (`node -e fetch` under `AbortSignal.timeout(5_000)` — with no route the embedded
...
- [x] The isolation is read from the daemon's record, never provoked with a connect: for each of the
...
- [x] From `worker`: `app`, `gate` and `db` unreachable by name and IP (kept from
...
- [x] The in-container `yahoo-finance2` import check (`:265-268`) runs in `worker` instead of `app`;
      [06](06-the-app-cutover.md)'s bundle grep stays
- [x] Every assertion on caps, uid, read-only root and published ports still passes with the
      networks in place

**Gates**

- [x] `npm run typecheck`, `npm test`, `npm run build` green on the merged tree — 91 files, 1778
      tests. `scripts/smoke-test.sh` **was not run**: it needs a Docker daemon and the tree it was
      built in has none, so CI's container smoke job is the only place its new assertions have
      ever executed. `docker compose config` was the local gate for everything declarative
