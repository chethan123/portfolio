...
- [x] `createYahooClient({ timeoutMs = 30_000 } = {}): YahooClient` — an object returned
...
- [x] Every call passes `{ validateResult: false, fetchOptions: { signal:
...
- [x] The 30 s is the client's own and fixed — no call takes a deadline: the crumb handshake is
...
- [x] `app/lib/price-provider.server.ts` deletes its `import("yahoo-finance2")` (`:619`),
...
- [x] `tests/yahoo-client.test.ts` (new) takes the client's own surface: the library's shape
...
- [x] `SYMBOL_PATTERN = /^[A-Za-z0-9.^=-]{1,15}$/` and `isWellFormedSymbol(value: unknown): value is
...
- [x] `PRICE_WORKER_SOCKET` joins `configSchema` (`:35-94`) as `z.string().min(1).default(…)`, the
      ...
      under `/tmp`
- [x] `export function loadWorkerConfig(env): WorkerConfig` beside `loadConfig` (`:121`): a second,
...
- [x] `tests/config.test.ts`: `loadWorkerConfig({})` answers the default path and throws nothing —
...
- [x] `export async function startWorker({ socketPath, yahoo, timeouts? })` — `yahoo: YahooClient`;
...
- [x] The server's bounds, a set: `server.maxConnections = 8`; `server.maxRequestsPerSocket = 1`
...
- [x] The routes, spec §3.2's table and nothing else — any other method or path is `400`. Bodies
...
- [x] Per-endpoint rate caps, a sliding minute in memory: the eleventh `quotes` call and the
...
- [x] The library call is the client's own `quote(symbols)` or `chart(symbol, { period1: from,
...
- [x] `GET /healthz` is `200 { ok: true }` with nothing consulted — the container healthcheck in
      [05](05-deploy-the-worker-alongside.md) calls it, and it must say "accepting requests", never
      "Yahoo is fine"
- [x] Logs: one line per non-`200` answer naming the endpoint, the status and the reason, stem
...
- [x] `Dockerfile:104-110` gains `server/yahoo-client.ts`, `server/symbol-pattern.ts` and
...
- [x] Each case starts `startWorker` on a socket path under `os.tmpdir()` — short on purpose: a unix
      ...
      not a client of it
- [x] `/healthz` answers `200 { ok: true }` with the fake untouched; `/quotes` with three symbols
...
- [x] `400` with no library call: a body naming `BRK/B`; a null element; an empty `symbols`; 101
      symbols; a body that is not JSON; a `history` body without `from`, and one whose `from` is
      `2024-6-1`; `GET /quotes`; `POST /other`
- [x] A 17 KB body: the socket is destroyed at the cap with no status — the request errors
      (`ECONNRESET` or `EPIPE`), and the fake is never called
- [x] Eleven `quotes` requests in one minute: ten calls, the eleventh `429` with no call; `history`
      at twenty-one the same
- [x] A client built with `timeoutMs: 50` over the signal-honouring `fetch` fake above, asked for a
...
- [x] A stale file at the path is unlinked and the server listens; a directory at the path makes
