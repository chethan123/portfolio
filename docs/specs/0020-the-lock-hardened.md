...
- **`/healthz.data` stays as it is.** The review's F2 — that a single-fetch request for the exempt
  health path runs the root loader and serialises setup state for a browser holding no grant — was
  judged by the owner on 2026-09-05 and kept deliberately: the request is behind the gate, the
  fields are setup state and never a figure, and the exemption's shape is not worth a second rule.
  Ticket [08](lock-hardening/08-bring-the-documents-level.md) corrects the comment in `app/root.tsx`
  that calls the data form harmless *because no data is served*, so the code records the decision
  rather than a false reason for it. No ticket changes the behaviour.
- **The order is the review's, minus that one.** Tickets 01–05 and the real-device walk in 07 land
  before the household's first passkey is enrolled; 06 and 08–11 may follow it.
- **A tab hydrated hidden keeps counting as hidden.** The review's F10 (a background tab opened
  ...
  prompt. Ticket 05 tells the family; ticket 08 says so in the module header. No code changes.

## Solution

Eleven tickets, each one pull request that typechecks, builds and carries its own tests. They fall
into four kinds, and the kind decides how a reviewer should read the diff:
...
[09](lock-hardening/09-close-the-third-interleaving.md) the third bootstrap interleaving,
...
**The small ones** — [11](lock-hardening/11-the-small-ones.md), three pull requests of nits that
...
- A ticket that changes a screen retakes the screenshots with `scripts/capture-screenshots.ts`
  (docs/README.md and docs/developing.md both make that the definition of finished). Only 05 does.

## Testing

The review's mutation pass is the yardstick: after ticket 02, every one of its nineteen mutations
must turn a test red for the right reason, and the four that survived (dropping `where not
exists`, writing `bootstrap = false`, adding `Domain=`, removing the acknowledgement check with a
valid assertion present) are named in that ticket as the boxes. The drive script under
[`harness/`](../research/2026-09-05-lock-slice-launch-review/harness/) is the manual check for
what the suite cannot reach — the `Layout` wiring — and ticket 02 says so rather than pretending a
DOM-less suite proves it.
...
- **A browser-driven test suite.** The suite is deliberately DOM-less (AGENTS.md); the one wiring
  that needs a browser is checked by the drive script, by hand, and ticket 02 records the gap.
...
- [`lock-hardening/09-close-the-third-interleaving.md`](lock-hardening/09-close-the-third-interleaving.md)
...
Blocked-by graph, so several can run at once: 01, 02, 03, 04, 06, 10 and 11(a) start on nothing;
05 waits for nothing but is where the guide is edited, so 07 waits for 05 and 08 waits for 05 and
07; 09 waits for 02 (its race test extends the pins 02 adds); 11(b) waits for 04 and 11(c) for 02.
