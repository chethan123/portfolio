...
- [ ] `availableFilters` no longer builds an owner facet, and nothing else regresses: the remaining
      six dimensions keep their two-or-more-values rule
...
**Tests** (extending `tests/holdings.test.ts`)

- [ ] The filter bar offers no Owner select, and the six other dimensions are unchanged
