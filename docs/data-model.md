...
written for someone holding a database dump who is extracting figures from it with plain SQL or
...
document retells all of them for a reader who may have none of them open. That duplication is
deliberate and named here, with the migrations the ones to believe.
...
- **Positions**: an account's holdings are recorded as immutable, dated photographs
  ...
  transaction ledger, so nothing records buys, sells, or transfers. No row is ever updated. The
  current portfolio is the latest photograph per account; the portfolio on a past date is the
  latest photograph at or before that date (DESIGN.md §3, §7).
...
priced at 1.00, and a loan is a negative quantity of the same instrument. Cash and debt therefore
travel the same code path as a share position, and net worth is one `SUM` with no special cases
(DESIGN.md §2).
...
- **Surrogate keys are `bigint generated always as identity`.** They are monotonic, which is what
  ...
  cookie's security. A sequential one would hand a bearer token an attacker could count through.
  It is the only surrogate key in this schema this rule does not describe.
...
- **Timestamps are `timestamptz`**, stored in UTC regardless of the container clock. Statement and
  close dates are plain `date`, because a statement describes a day, not an instant.
...
**`person`** is a family member, a name and nothing else. Not a login; admission to the instance is
the sign-in gate's business, and a person row is purely whose money a screen says something is.
...
**`account`** is one account at one institution, owned by exactly one person. Joint ownership is
...
- **Tax treatment is three-way, never a boolean**: $500k tax-deferred and $500k tax-free are
...
**`classification`** holds the user-editable category labels (they grow over time), each rolled up
to a fixed asset class so mixed-axis labels still aggregate into a stock/bond/cash split
(DESIGN.md §4.4).
...
**`instrument`** is anything a quantity can be held of: a listed security, a workplace-plan trust
...
**`position_set`** is an immutable, as-of-dated photograph of what one account held. Every upload
...
a `psql` session, as [`importing-history.md`](importing-history.md) shows, and
the set's holdings go with it by cascade. No screen offers the delete.
...
`latest_position_set` scans, so "which set speaks for this account" is an index scan stopping at
the first row. `id` makes equal timestamps deterministic.

**`holding`** is one instrument's line inside one position set.
...
**`quote`** is the live price behind current-holdings screens.
...
**`price_daily`** is the spine. A refresh files each price under the day the provider says it
belongs to. Usually that is the running session, whose row is rewritten through the day and settles
on the close. Sometimes it is the previous trading day, a fund's evening NAV fetched next morning
or a holiday poll returning Friday's close. A row is thus only ever rewritten with the provider's
own price for that same day, which makes the rewrite idempotent unless the provider itself revises
a close, which is a correction rather than corruption
([`app/lib/prices.server.ts`](../app/lib/prices.server.ts) argues this in its header). Non-trading
days get **no row**; every historical query carries forward the last close at or before the asked
date, so Saturday equals Friday and a market holiday equals the trading day before it, with no
calendar table anywhere.
...
| `fetched_at` | `timestamptz` | no | when we learned it, a fact about us and distinct from `as_of` |
...
**`price_poll`** is one row per refresh attempt, successful or not. Because unchanged quotes write
no observation, a silence in the log is ambiguous: a quiet market, a failed provider, and a stopped
server look identical. This table is what tells them apart later.

| Column | Type | Nullable | Meaning |
|---|---|---|---|
| `id` | `bigint` identity | no | primary key |
| `started_at` | `timestamptz` | no | when the attempt began (an attempt that never commits leaves no row at all, stated and not solved) |
...
| `mapping` | `jsonb` | yes | null until the columns step passes, so "how far did this draft get" is a property of the row, not a status column |
...
**`app_setting`** is the household's settings, one row enforced by the schema. The primary key is a
...
| `refresh_cadence_minutes` | `integer` | no | default `15`, CHECK 1–1440; how often prices refresh. Quotes are asked for from 15 minutes before through 15 minutes after regular market hours, and a backfill batch runs beside them at any hour while some spine still has a gap |

**`schema_migrations`** is the ledger, created by the runner
...
| `backup_eligible` | `boolean` | no | eligibility for backup, fixed at enrolment and never re-read. It means the credential *can* sync, never that a copy already exists |
...
| `last_used_at` | `timestamptz` | yes | null until this passkey first authorises anything: `verifyScopedAssertion` (`app/lib/lock.server.ts`) writes it on **every** verified assertion, not only an unlock. That includes the "prove yourself" assertion that authorises enrolling another passkey or removing one |
...
rather than arbitrarily. The ordering matches `position_set_account_as_of_idx` exactly. `STABLE`,
...
- The price is the greatest `price_daily.close` at or before `d` (the carry-forward; LEFT lateral
  ...
  `is_stale` is constant `false`, because a historical close is the close. `annual_dividend`
...
This is who writes what, useful both for auditing a dump and for rebuilding the service:
...
- **The price refresh** overwrites `quote`, appends new `price_observation` instants, keeps each
  market day's `price_daily` row at the provider's latest price for that day, and records the
  attempt in `price_poll`. It also refreshes `instrument.quote_type` from the provider.
  [`app/lib/prices.server.ts`](../app/lib/prices.server.ts) is the only price writer, taking the
  provider through the `PriceProvider` interface
  [`app/lib/price-provider.server.ts`](../app/lib/price-provider.server.ts) defines, and
  [`server/yahoo-client.ts`](../server/yahoo-client.ts), reached from the price worker process
  rather than this one, is the only importer of the provider library. The **backfill batch** runs on
...
- **Application writes append** `position_set`, `holding`, and `price_observation` rows. There
  ...
  price changes, and metadata edits can change them. Manual history is operator-managed, and a
  `price_daily` row is only ever rewritten with the provider's own price for its day, so a finished
  day changes only if the provider revises its close. Overwritten in place: `quote`, `app_setting`,
  an `upload_draft` as its steps write parts back, `account`'s editable columns (name, institution,
  kind, owner, tax treatment and `external_account_number` from the Settings edit. A commit
  also captures an account number where the column is empty. Then `closed_at` when it closes; note for
  a dump auditor that a historical position set reports under the account's *current* owner), and
  `instrument.quote_type` on each refresh. `instrument`'s symbol, name and classification are
  mutable by design, so that a ticker change is a one-column update (DESIGN.md §4.3), though nothing
  writes them today.
...
**Current net worth.** One sum with no branches (cash and liabilities included by construction):
...
never an operand, because `price` is the only column there anything may compute from); mix
`manual_networth` into computed figures (it is a separate chart series that loses to computed values
on overlap); treat a missing `price_daily` row as a zero (it is a non-trading day, so carry
forward); or coalesce a null `cost_basis_per_share` (the null means *untracked*, and zero would
fabricate a gain).
