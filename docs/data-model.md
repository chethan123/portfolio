...
live schema, regenerated by `npm run db:types` and verified in CI. This document retells all three
for a reader who may have none of them open — a deliberate duplication, named here.
...
- **Prices**: each instrument's price history is a spine of daily closes (`price_daily`), one row
  per finished trading day, plus a single overwritten current quote (`quote`) and an append-only
...
Five tables stand alone, referencing nothing:
...
**`price_daily`** — the immutable spine. Non-trading days get **no row**; every historical query
carries forward the last close at or before the asked date, so Saturday equals Friday and a market
holiday equals the trading day before it, with no calendar table anywhere.
...
reader of these is [`app/lib/valuation.server.ts`](../app/lib/valuation.server.ts); a screen
...
flowchart LR
    subgraph ingest [Ingest]
        ...
        MB[manual balance / correction] -->|app/lib/positions.server.ts| PS
    ...
    subgraph pricing [Pricing — app/lib/prices.server.ts]
        ...
        P --> PD[price_daily: append finished days]
...
- **Manual balance entries and position corrections** write a new `position_set` (`source =
  'manual'`, no file) — a correction is a new photograph, never an edit
  ([`app/lib/positions.server.ts`](../app/lib/positions.server.ts)).
- **The price refresh** ([`app/lib/prices.server.ts`](../app/lib/prices.server.ts) — the only
  ...
  instants, files finished days into `price_daily`, and records the attempt in `price_poll`.
...
- **Never updated**: `position_set`, `holding`, `price_daily`, `price_observation`,
  `manual_networth` rows are append-only facts. Overwritten in place: `quote`, `app_setting`, and
  the mutable naming columns of `instrument` / `account` / `classification`.
