...
flavour because its healthcheck needs `wget`, which the default distroless image does not carry.
...
allowlist whatever its mode and owner; and `NET_BIND_SERVICE` on `caddy`, which its binary needs to
`exec` at all rather than to bind anything. `scripts/smoke-test.sh` asserts all of it against a
running stack. Nothing else would notice a container quietly regaining root.
...
**`./volumes/db/data` is the whole of the state, and it is a directory you can see.** It holds every
...
one-megabyte `tmpfs`, memory rather than disk, that `worker` and `app` share only to meet at a unix
...
refusal both work. Read it before the first `up`. The public origin your house proxy will serve this
instance at has to be decided first, because the redirect URI is built from it.
...
There is no domain rule and deliberately no option for one, because the narrowest domain that would
admit this household also admits every other Gmail account on earth. The file is the only
authorization this instance has, and everyone on it sees and can do everything. Editing it is
...
(data will be lost)?` It asks because the volume record holds the old absolute path. Answer `y`.
What gets recreated is that pointer, not the directory; this is the one configuration where the
prompt's warning does not apply, and `docker compose down -v` is harmless for the same reason.
...
- The bundled `db` service still starts, still initialises `./volumes/db/data`, and `app` still
  ...
  command. It gates `db` and `dump` behind a profile nothing sets by default, so neither starts, and
  gives `app` a fourth network, `external-db`, that carries a default route which no other network
  here does, because reaching a host outside this Compose project needs one. Forgetting the
  ...
  with the override loaded, even though `db` never starts under it, because `compose.yaml` still
  interpolates its value for the `PGPASSWORD` `app` reads. Generate one regardless.
...
`compose.external-db.yaml` adds so `app` can reach a host outside this Compose project, adds a
default route to `app`. The ordinary stack already gives external routes to Caddy, the gate, and the
egress proxy; it withholds them from `app` and `worker`.
...
compose file that leaves one out starts everything else correctly and never starts it. There is no
error and no unhealthy row, an *absent* row instead, the one nobody counts. Leaving out
`egress-proxy` alone is the quieter break of the two. `worker` still starts and still answers its own
healthcheck green, and now sits on an internal network with nothing on the other end, so every price
fetch fails at the first hop, and nothing in `docker compose ps` says why. And `/healthz` answering
`200` with exactly:
...
a transition (the worker dying or coming back) can lag by that much. An uncached answer is worth
having right after this `up -d`, and again after any change to the host's engine or container
runtime. An SELinux-enforcing host, `userns-remap`, or rootless Docker can each leave the worker's
socket permissions different from what this stack assumes, and none of it is proven anywhere in this
repo's CI, because its runner is none of those. Dial the same hop by hand:
...
`0` means `app` reached `worker` over the shared socket end to end. For anything else, see
[Monitoring](#monitoring). That includes a silent hang, which the `timeout` above turns into a `1`
within five seconds instead of blocking forever, the same guard `worker`'s own healthcheck in
`compose.yaml` carries. That `5000` is a once-only, socket-inactivity timer on a probe that only
ever runs by hand or at deploy. Keep it straight from `pricing.worker`'s own deadline, a much
...
The complete set of knobs an operator has. Every deployment setting is an environment
...
Settings → Prices, in whole minutes, from 1 to 1440, default 15. The automatic poll still runs in
the app process, and its *quotes* are asked for from 15 minutes before through 15 minutes after
regular market hours, while the backfill batch beside them rides a tick at any hour and only while
some spine still has a gap. The **Refresh now** control on any figure screen spends a request at any
hour either way. An environment that still sets the old `PRICE_POLL_INTERVAL_MINUTES` is ignored
without error. If you had tuned it, re-enter the value once on that screen after upgrading.
...
`docker compose up` stops on the first one that is unset *or empty* and names it. That happens
before a container exists, which is why the message is Compose's rather than the startup validator's
at this stage.
...
- it has to be a bare origin: no path, no query string, no fragment, and never
...
value against what the browser actually sent, as plain text, and neither side is renormalised first,
...
**Changing this value to a different hostname does more than move the gate's redirect.** The lock
derives its relying-party id from it too, the bare hostname, via `expectedRelyingParty` in
...
lists it came along for the move. If the household holds any passkeys, moving to a new hostname
takes more than this variable and the registered redirect. Remove every enrolled passkey as part of
the move (the command is in
...
variable is validated at startup. Compose resolves them before a container exists, so a typo
surfaces as a failed pull, not as the message naming the variable that the settings below get.

**An empty value reads as unset, not as "configured to empty", on both sides and in opposite
...
less protection than it has, never more. For the gate's own settings, empty is a refusal to start.
Neither can leave you quietly unprotected.

**`MAX_UPLOAD_MB` does not reach the container under the bundled Compose file.** The application
validates and reads it, but it is missing from the `app` service's `environment:` block, so setting
...
published port 80 directly and land on the bundled Caddy, skipping the house proxy entirely. That
device is the whole reason the gate exists. So the gate travels with the app, in the one
...
or a second hostname, holds no cookie the gate will accept, and is bounced to Google and back to
...
- **Give the site block a real hostname** instead of `:8080` in the `Caddyfile`, so Caddy requests
  ...
  that: the ports it listens on internally, with the public ones forwarded onto them. They do
...
The `trusted_proxies` line stays correct either way, because with no proxy in front nothing sends
`X-Forwarded-*` for Caddy to believe.
...
account key and every certificate it has ever issued. Recreating the container throws all of it
away, and recreating the container is exactly what the `docker compose up -d` in
...
target and says `target /data already mounted as services.caddy.tmpfs`, so the whole `tmpfs:` block
goes:
...
directories, `/data/caddy` and `/config/caddy`, are world-writable, so uid 65532 writes them from the
first start. (The root-owned `0755` parents are why the tmpfs entries carry `mode=1777`, since a
tmpfs seeds nothing.) A volume created any other way must have `/data/caddy` writable to 65532. An
...
External monitors move to `https://<hostname>/healthz`, because plain-HTTP `/healthz` now answers
only from inside the container.

### Forwarded headers

The gate **trusts `X-Forwarded-*`**, and the bundled Caddy sets them itself, so there is nothing to
configure there. The application now reads no forwarded header at all; the two it used to read went
with the password it served.
...
- **Nothing is stored on the phone.** The service worker is network-only, with no cached screens and
  no last-known figures. Off the VPN, the installed app shows a branded connect-the-VPN page and
...
- **A LAN address does not install.** Service workers require a secure context; the house proxy's
  ...
  none. The gate refuses those requests anyway.
...
Every request that arrives at the bundled Caddy goes to the `gate` sidecar before Caddy will forward
it. That includes routes that do not exist yet, and the JavaScript bundles, the CSS and the font,
which the old in-app gate left open. The sidecar admits a request only if all of this
holds:
...
see it, because `compose.yaml` tells the app a gate fronts it. **If that banner ever appears on your
...
so brute force moved to Google's own sign-in, which does have rate limiting, lockout
...
lifetime is the seven-day default. That default is what makes signing in a once-per-device event
...
uid here would be the worse choice. Pinning one would decide on your behalf that your allowlist file
is readable by that uid, and hand you a sidecar that will not start over a file mode nobody
mentioned. What bounds it: every capability dropped but `DAC_READ_SEARCH`, which is exactly the root
power being kept, opening a file it does not own whatever its mode. Alongside that it runs with
`no-new-privileges`, a `read_only` filesystem with a tmpfs `/tmp`, no volume and no published port.
Uid 0 holding one capability is a far smaller soft spot than root was, and it is still written down
...
outbound requests to exactly one destination: the price provider. What goes out is the list of ticker
symbols being priced, plus your public IP. Quantities, balances, account names, people and filenames
do not. There is no analytics or error-reporting SDK anywhere in the image. The symbol list still
...
only on port 443 and only to the five hosts `yahoo-finance2` 4.0.2 can reach:
`query1.finance.yahoo.com`, `query2.finance.yahoo.com`, `finance.yahoo.com`, `guce.yahoo.com` and
`consent.yahoo.com`. Nothing else from `worker` goes anywhere. The `CONNECT` host alone would not be
enough of a check, because all five names resolve to the same handful of addresses that also answer
for unrelated Yahoo properties (mail, sign-in, the main site), so an address match proves nothing
about which one is actually being reached. The edge decides that from the server name the *client*
presents. So the proxy checks that too, once the tunnel is open. It reads the TLS ClientHello's own
`server_name` and refuses to relay a single byte further unless it matches the host that was
`CONNECT`ed to, which a compromised or misdirected `worker` cannot fake without the edge itself
agreeing to answer under the wrong name. Every refusal, whichever check failed, is one line in
`egress-proxy`'s own log, from `docker compose logs egress-proxy`, reading
`Egress proxy: refused CONNECT <host> — <reason>`. A host outside the five reads
...
than answer with a status, and the text that reaches `app` under `Price provider failed` (see
[Logs](#logs)) is
`fetch failed: Client network socket disconnected before secure TLS connection was established`
rather than a numbered failure. `worker` stays healthy throughout every one of these,
because its own healthcheck asks its own socket and never Yahoo or the proxy. The allowlist itself
is a constant in `server/egress-proxy.ts` rather than a setting, with no environment variable or
file to add a host to, so if Yahoo ever moves `guce.yahoo.com` or `consent.yahoo.com` to a new name
(a snapshot of Yahoo's live redirect chain, unlike the three `finance.yahoo.com` names the pinned
library hardcodes), quotes stop and the refusal above names the host Yahoo redirected to instead.
The one fix is a release that edits that constant; there is nothing here to configure your way out
of.
...
- [ ] Accept that you are now responsible for patching what faces the internet, including the
...
second. If that is not possible either, deleting every passkey returns the instance to open, and the
household can enrol again from a provider whose passkeys reach other devices.
...
locked from the next request it makes, not on its next visit to Settings and not after some delay.
...
the stop taking hold. A registration that finishes inside that gap writes a passkey. The
challenge was minted while the household still held one, so the server accepts the completed ceremony
as an ordinary later enrolment rather than a first one, with no check required, and the instance is
...
lock derives its identity from this value too, and a hostname change orphans every passkey the
...
pinned by a test (`tests/routes/healthz.test.ts` asserts it key for key, so a rename cannot
...
`database` before `migrations`, and read `migrations` only when `database` is true, because the
migration ledger lives *in* the database, so when the database is unreachable there is nothing to
read and the field still says `current`. **`migrations` is meaningless whenever `database` is
false**, and the third line above is what that looks like. The second line above is the ordinary
shape of a healthy instance whose worker container is down, restarting, or unreachable over the
socket. `pricing`, all four of its keys together, never turns `200` into `503`, by design (spec
price-health/02 and /03), so read it as its own signal, worth its own alert rule and not folded into
the one for `status`, rather than expecting it to explain a non-`200` response.

**`pricing.worker`** is `available` or `unavailable`. `"available"` proves only that this process
reached the worker's own `GET /healthz` over the socket; it proves neither `egress-proxy` nor
Yahoo, both of which fail while `worker` still answers `200` (see [Logs](#logs)). It is also cached
for up to five seconds per app process, so a monitor polling faster than that can see a transition
lag behind the real state by that much.

**`pricing.scheduler`** is `not_started`, `running`, `on_schedule` or `overdue` (spec
price-health/03), read from the poller's in-process state. Root middleware first runs the idempotent
scheduler bootstrap; the loader reads its snapshot without retiming it or calling the provider.
`on_schedule` means armed and not yet late, nothing stronger. **A fresh container reports
`on_schedule` for up to one full refresh
...
ever run. `not_started` means this process holds no poller slot at all, and on this endpoint that has
exactly one cause: `startPricePoller` threw. The middleware that arms it runs ahead of every request
the app serves, `/healthz` included (`app/root.tsx`), so a response carrying `not_started` has
already had the call made in that same request. There is no "hasn't started yet" left to rule out.
Grep the stem `Price poller did not start`, which that failure always logs. `overdue` means the timer
has not begun a tick in more than the cadence plus a five-minute grace, measured from when a tick
last *started*. It covers both a stopped timer and a tick that started and never returned, on
purpose, since a hung tick would otherwise pin this at `running` forever (see
[the runbook](runbook.md#prices-have-stopped-updating)
...
**`pricing.quotes`** is `not_attempted`, `market_closed`, `ok`, `partial`, `failed` or `unknown`
(spec price-health/03). It carries the last tick that observed a provider outcome, either a scheduled
tick or the one an upload requests once its transaction commits; a direct **Refresh now** press does
not update it, since that route calls the refresh machinery directly rather than through the poller.
`not_attempted` means no poller slot, or a slot that has not yet recorded one. **A weekend, or any
hour outside the scheduled quote window (§6.2, market hours padded ±15 minutes), reports
`market_closed`.** The tick still ran,
...
the socket by this combination alone. `unknown` means the tick's own database or lock work failed,
not the provider.
...
**Writes failing.** The probe is `select 1`. A full disk, a read-only tablespace or a revoked
`insert` grant all leave that succeeding. The container is healthy, the dashboard is green, and
...
able to *act* and not only to alert. Otherwise you have to be the one who acts.

### The worker's own healthcheck

`worker` carries a healthcheck of the same shape as `app`'s: `node -e`, no shell, on an interval. It
is reached over `/run/price-worker/worker.sock` instead of a port, since the container publishes
none. Its `/healthz` consults nothing. It answers `200` the moment the process is accepting requests,
before it has made a single call to Yahoo. So "unhealthy" here means exactly that and nothing wider.
The worker is not accepting requests on its socket, ordinarily the process gone or wedged. It never
means "Yahoo is failing," which is a different thing entirely and shows up instead as stale prices
and `app`'s own `Price provider failed` line, with `worker` reporting healthy the whole time. Nor
does `/healthz` spend from either endpoint's own rate budget, since the branch above answers before
either `admitQuotes` or `admitHistory` is ever consulted. So a worker whose ten quotes or twenty
history calls this minute are already spent keeps answering `200` while every real call over
`/quotes` or `/history` comes back `429`. That is the identical all-green, no-prices shape this
subsection exists to warn about, and post-cutover the one place it shows at all is `app`'s own log,
not this healthcheck (see [Logs](#logs)). And the restart rule above still holds. Nothing here
recreates `worker` on this failing, so an unhealthy row in `docker compose ps` is where you look, not
something Compose resolves for you.
...
*process* limit kills nothing. The kernel refuses the next thread or process, so the worker
stays up, keeps answering `/healthz`, which starts no threads, and fails the work instead. The one
tell is in the logs. The worker writes `Price worker listening on …` once per process start, so
seeing it more than once is the process having started more than once; see [Logs](#logs).
...
a ninth socket and closes it again within a couple of milliseconds, with no error and no timeout.
That is a TCP handshake that completes cleanly whether or not anything behind it still has room to
...
second half of the pipeline proper, and the only place a refused sign-in is recorded at all, since
the application no longer sees one. The stems below are for grepping and may drift, because the code
owns the wording:

- **One line per HTTP request** from the server's built-in request logger: method, path, status,
  duration. The container healthchecks, the app's own and Caddy's, hit `/healthz` every
...
- **One line per refresh the poller actually runs**, stem `Price refresh`. Informational when
  ...
  line at all, and [below](#there-is-no-price-line-in-the-log-has-three-causes) lists which silences
  are ordinary.
...
- **A provider outage** at error level, stem `Price provider failed`. Every selected instrument
  ...
  permission slip. `docker compose ps` shows `worker` unhealthy, restarting, or missing
  ...
  answers `/healthz` with `200`. Since this release that "Yahoo error" is at least as often
  `egress-proxy`, which is `worker`'s only route anywhere, as Yahoo itself, and the two read
  differently. `docker compose ps egress-proxy` not `healthy` means every call fails before it ever
  reaches Yahoo: `fetch failed: connect ECONNREFUSED <address>` for a proxy that has stopped
  answering, or `fetch failed: getaddrinfo ENOTFOUND egress-proxy` for one the network cannot name
  at all. That second one is as often a proxy that was never *started*, a partial `up -d` that named
  services explicitly and left this one out, as one whose network broke. `docker compose ps` showing
  six rows where seven belong tells those apart, and the broken network is much the rarer.
  Either way `worker` answers `app` with a `502`, and, confusingly, `ECONNREFUSED` names a
  different actor than the `ECONNREFUSED` under `no worker listening` above: there it is `app`'s own socket to `worker` going
  ...
  transaction) log `Price refresh failed`, a pressed **Refresh now** included. There is
...
- **Startup**, in order: the configuration check, one line per migration file (applied or skipped),
  ...
  server is never started. The stem is `Migrations`.

`worker`'s own log has one stem, `Price worker`, with one line per non-`200` answer over the socket,
...
- **A session refused**, stem `Invalid authorization via session`, is someone whose address is no
...
or not anyone has opened it. The root middleware arms the poller from the container's own
healthcheck traffic, not from a page view. (The timer boots at the seeded 15 minutes and picks up a
...
first, a copy of `./volumes/db/data` is a valid snapshot, and restoring one means putting the
ownership back
...
and an unattended script that ignores the exit status will collect those for months. Check
...
has a copy of them. Losing `.env` entirely has a first symptom none of what follows fixes.
...
- `GATE_CLIENT_ID` and `GATE_CLIENT_SECRET` are recoverable, but only from the Google Cloud console,
  and the secret may have to be regenerated there rather than read back.
- `GATE_COOKIE_SECRET` is recoverable by regenerating it, which signs the household out.
- `POSTGRES_PASSWORD` is recoverable with no outside help at all, and the one most likely to be
...
disk. Compose interpolates the whole file before running any verb after that, `exec` included, and
refuses on the first variable `.env` does not yet hold, with the `>` redirect above still
...
tell you. `app` still fetched its own prices under the old file, and `worker` idled unused.
That grace ended with that release, and `app` has dialed `worker` instead of fetching for itself ever
since. Start today's image under a `compose.yaml` from before that point and there is no volume and
no worker. Prices go stale while `/healthz` keeps answering green, since a missing price provider was
...
the service it targets has not itself changed. `dump` has changed in this release: a new network,
...
than copying this line unchanged next time. `worker` and `egress-proxy` have no `depends_on` tying
them to anything, and nothing depends on them, so naming each explicitly is what starts it at all. A
list that left one off would look exactly like success, everything named coming up healthy with the
...
cross from `1` to `2`, because a major is where a breaking change would be. Read the release notes,
set `APP_VERSION=2` in `.env`, then run the same procedure above.
...
3. Generate the password with `openssl rand -hex 32` and write it to `.env` as
   `POSTGRES_PASSWORD`. Skip this and step 4 refuses by name, with the message `compose.yaml`
   carries pointing back to this section, rather than starting anything half-configured.
...
4. Change the role, while the old containers are still running on it:
   ...
   believe the password is. From the moment it runs, any *new* connection either of them opens
...
5. `docker compose up -d`, which recreates every service that release changed: `db`, `dump`,
   `app`, `gate` and `caddy`, each gaining a network there; `worker` alone was untouched by it.
   `caddy` is one of the five, and it is the one that publishes `80:8080`, so the outage
...
Migrations are forward-only: there are no `down` files, there is no rollback
command, and the ledger records filenames only, with no checksums and no content hashes. Two
...
match the pinned image, or do not pin below this release at all and re-upgrade instead. This slice
adds no migration and no column, so nothing it wrote to the database is at risk either way; an old
image only loses the worker cutover, never any data.
...
back to match the pinned image, or do not pin below this release at all and re-upgrade instead. This
...
The volume was left under its old name deliberately, so this is a copy rather than a move. The
original is still there afterwards, and the way back is to stop and delete the new directory.
...
sure, `docker volume rm portfolio_db-data`. That volume is the last copy of anything you did not
also dump.
...
2. `docker compose down`, then empty the data directory deliberately, because `down -v` will not,
   and `0700` uid 70 means your own account cannot either:
...
The demo household in [`scripts/seed-demo.ts`](../scripts/seed-demo.ts) is two people, six accounts,
sixteen instruments and three years of statements. Measured against it, the whole database is about
...
against your own instance rather than against this table.** The table is the design target, not a
measurement:
...
purpose, because it is what makes an old import auditable. A brokerage CSV is tens of kilobytes and
the design target is a handful of uploads a quarter. Abandoned upload drafts are swept once they are
a day old, but only when the *next* upload starts. There is no scheduler, so an instance nobody
uploads to keeps its last abandoned draft indefinitely.
...
caps this by configuring the `json-file` driver for every service with `max-size: "10m"`
...
**Almost no resource limits are set.** There is no CPU limit anywhere, and no memory or process
limit on any service but `worker` and `egress-proxy`, which each carry `mem_limit: 256m` and
...
trust than `worker` itself. On a machine that runs only this, that is the right default. On a
...
statement uploads a quarter. It says that almost nothing behind it is benchmarked, the 1D
read being the one exception. If your instance is much larger than that, §10 is also the
