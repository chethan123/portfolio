...
| `input[type="file"]::file-selector-button` | The picker's own button, which is otherwise the browser's: 28px tall inside the 40px box, transparent, 1px `--outline-variant`, `--primary` text, 4px radius. `.button--quiet`'s treatment at the scale of a control nested in a control — the chip radius rather than the button's, because an 8px corner inside an 8px corner reads as two edges arguing |
