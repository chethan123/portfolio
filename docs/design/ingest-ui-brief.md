...
| `input[type="file"]::file-selector-button` | The picker's own button, which is otherwise the browser's. Drawn as the field's **leading action**, not as a button inside a field: full height, flush to the leading edge, `--panel-hover` ground, `--primary` text, no radius and no border but one `--outline-variant` hairline dividing it from the filename. On hover, one more step of the same ramp, mixed toward `--on-surface` rather than named — `--secondary-container` is the app's "you are here" ground and not available to a hover. The box keeps its standard control treatment, so `aria-invalid`'s border and the focus ring stay on the control being operated |
