...
| button modifiers | `--quiet`: transparent, 1px `--outline-variant`, `--primary` text. `--text`: no box, `--primary`, underlines on hover — it keeps `.button`'s 40px so a text action stands on the line of the button beside it. `--danger`: 1px `--loss`, `--loss` text. `--block`: full width |
...
| `input[type="file"]::file-selector-button` | The picker's own button, which is otherwise the browser's. Drawn as the field's **leading action**, not as a button inside a field: full height, flush to the leading edge, `--panel-hover` ground, `--primary` text, no radius and no border but one `--outline-variant` hairline dividing it from the filename. `--secondary-container` on hover. The box keeps its standard control treatment, so `aria-invalid`'s border and the focus ring stay on the control being operated |
