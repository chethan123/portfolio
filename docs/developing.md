...
that have to commit (the two-connection races in `lock`, `lock-schema`, `account-lock` and
`net-worth-change-race`, and one `price-backfill` case) sweep their own rows, so the suite leaves
the database exactly as it found it, including after a failure.
