...
- **The solid computed line** is derived, on every read, from two ingredients. *Positions* are dated
  statement photographs in `position_set`, one per account per statement. *Closes* are one price per
  instrument per trading day in `price_daily`, carried forward across non-trading days. A past
...
Recording it as a single cash row would file the equity side of the household under cash in every
as-of valuation behind the charts, while the coverage counts report the figure as fully known.
Where this document says "manual series" for something you might wish were computed, that rule is
why.

This section is the third telling of the two lines. [DESIGN.md §7](../DESIGN.md) holds the rules,
and [`guide/overview.md`](guide/overview.md) reads them off the screen. The duplication is
deliberate, because an importer has to hold both in one place before deciding where anything goes.
For every rule's reason, those two are the authorities.
...
| Historic holdings | Nothing | The dashboard has nowhere to capture point-in-time holdings from. The computed line's past comes from brokerage statements, or not at all |
...
Other holdings with price coverage still count, including cash and loans priced at 1.00. The chart
draws those partial points on the ordinary solid line with nothing beside them saying so. That is
the half of issue #83 the backfill does not answer, and
[issue #216](https://github.com/chethan123/portfolio/issues/216) carries it. A large import can take
several refreshes. Provider failures, unsupported instruments, or missing provider history can leave
gaps unresolved; check the outcomes at Settings → Prices.
...
4. For investment accounts, upload the **most recent** statement first, then the older ones. The
   most recent creates the instrument rows and classifies them. The older ones may read short for a
   refresh or two while their closes arrive.
...
- **These are unofficial endpoints.** Names, paths and field names have drifted across Empower's
  ...
  API, including this document's claims about what the dashboard does and does not serve. That is
  why every recipe below tells you which fields to identify by eye rather than assuming a name.
...
[`guide/overview.md`](guide/overview.md) explains what a reader of the chart is told about it. You
load the series in the terminal instead, into the `manual_networth` table: one row per date, two
...
Leaving overlapping points in place is harmless; importing them is pointless.
...
substituting the path and field names your capture actually uses. A month missing its first day is
skipped, which a rough prefix can afford. To thin daily data to one point per month, keep the
day-one rows:
...
**Read the file before loading it.** It is short, which was the point of thinning. This is the
...
A re-run fails loudly rather than doubling anything, because `date` is the table's primary key. The
...
entered in any order, since each carries its own date. A loan is typed as the amount owed, and the
form applies the sign.

**Decide whether the per-account depth is worth the typing before you start.** The manual series
from step 2 already covers the household total over those years, and the computed total wins on any
date it covers. So a checking account loaded back through those years makes the *computed* line
start earlier, carrying only that account until the investment statements reach that far. What
per-account balance history buys is that account's own page reaching back, and an honest cash line
through the years before the app. For a household that mostly wants the total, step 2 alone is a
perfectly good answer, and this step can cover only the recent past, or nothing.

## Step 4: investment accounts, from brokerage statements

Empower cannot help here, because it has balances, not holdings. Your brokerages can, though. Every
...
- **Correct a mistake by re-uploading.** There is no screen for deleting an upload; a corrected
  ...
  in `position_set` by account and date (of the two rows a corrected date holds, the superseded one
...
And the record of what was tried, which holds an outcome's reason:
...
**Two traps to know about, now that the application handles them rather than you.**
...
by hand is among the tabs Settings lists as not built yet, so until it is built, do these in `psql`:
...
recorded live.** Use trading days only; a row for a weekend or holiday would state a close that
never happened, where the carry-forward already answers those dates honestly.

## Step 6: verify

**In the terminal.** You can ask the same function the dashboards read how well any date is
covered:
...
Ask it at a few dates spread across the era your backdated statements cover. `unpriced` should be
zero everywhere, except holdings you knowingly left without closes, and anything the backfill has
not reached yet. This terminal check is the only one that counts while a gap is open. No screen yet
distinguishes a past date priced worse than today from one priced fully. The Overview's coverage
sentence counts what is unpriced *now*, not then, and the chart-side warning that would say
otherwise is [issue #216](https://github.com/chethan123/portfolio/issues/216). Settings → Prices
answers the other question, which is whether anything is still missing at all.

**On the screens.** The Overview at the All range should now read continuously: dashed manual
...
same question. A mismatch worth investigating is a *large* one, which almost always means a missing
statement, a missing account, or a spine gap the queries above will name.

## Clean up

Delete the captured files: `histories.json`, any per-account history captures from step 3, the
accounts and holdings captures, and any TSVs derived from them. Everything they held that matters is
now in the database, which [`operating.md`](operating.md) already tells you how to back up; what
remains in them is account numbers in plain text with no further use.
