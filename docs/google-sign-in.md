...
Running the instance day to day is [`operating.md`](operating.md): the proxy in front, backups,
upgrades, and the security decisions that are yours. When something is broken and you want a
procedure, that is [`runbook.md`](runbook.md).
...
character. Changing your mind afterwards means editing the console and `.env` together.
...
`https://`, because the gate's session cookie is set `Secure`. TLS is the outer proxy's job
...
In the console's OAuth configuration, currently under **APIs & Services**, on the pages Google
groups as its auth platform, set up the screen a person sees when they sign in. Those labels have
moved more than once. The decisions have not. What has to be right:
...
that is unset or empty. Half-configured is not a state Compose lets by. It has one blind spot, next.
...
> one is read with `.env` open. `.env.example` sits next to the code and is the one to believe if
> they ever disagree.

## Step 5: the allowlist

Who may enter is not a variable. It is `allowed-emails.txt`, beside `compose.yaml`, one Google
address per line, bind-mounted read-only into the gate and re-checked on every request rather
...
admit this household also admits every other account on that domain. In the ordinary case that is
every Gmail account on earth. The file is the only authorization this instance has, and everyone on
it sees and can do everything.
...
Those prove the stack. The last check is Google, and only a browser can do it:
...
The restart is not what makes the address work, because the gate watches the file. But a
...
sessions are its own encrypted cookies, and `GATE_COOKIE_SECRET` is what clears them.
...
missing line. See [`runbook.md`, Nobody can sign in](runbook.md#nobody-can-sign-in).
...
anything. See [`runbook.md`](runbook.md#docker-compose-up-refuses-to-start-anything).
