...
// Most dates a chart checks for one range (spec 0009, issue #74). A span fitting the budget samples every day; wider spans get exactly this many, decaying geometrically backward from `until`.
...
// manual is read only for the household — hand-typed pre-app history was never any one account's (CONTEXT.md).
...
export interface RangeWindow extends Window {
  range: RangeKey;
  custom?: CustomSpan;
  // Present only when 1D actually resolved — its presence, not a separate flag, tells a loader to read the intra-session series.
  session?: IsoDate;
}
...
// Calendar-month arithmetic, not a fixed day count: 1M/3M/1Y/5Y are trailing calendar spans back to the same day-of-month. setUTCMonth's rollover (not clamping) is accepted as-is.
...
// Excludes "all"/"custom" (need the surface's earliest date) and "1d" (needs the latest session) — the compiler then demands a branch for each below.
...
// Household: earlier of position-set and manual (manual reaches furthest back). Account: its own position-set date only.
...
// Solves r > 1 in 1 + r + ... + r^(n-1) = target by bisection (ADR-0003): the sum is continuous and strictly increasing in r.
function solveGrowthRatio(n: number, target: number): number {
  ...
  // At the shipped budget this loop never runs (179 terms already sum past 1e53); kept for correctness if SAMPLE_BUDGET is retuned far downward.
  let low = 1;
  let high = 2;
...
// "all"/"custom" need the surface's earliest date rather than a calendar offset; an unusable custom span falls back to the default rather than erroring.
export function resolveRange(
  ...
  if (range === "1d") {
    ...
    // dates empty on purpose: 1D bypasses the day sampler, plotting the log's own instants. since is
    // the day before the session (today's price_daily converges on the last observation — this is the previous close). Known gap (§14): 1D holds today's positions constant while the change reader compares against since.
...
// A preset landing exactly on the surface's earliest date is not disabled — that day is a real, drawable start. "All"/"Custom" are never disabled.
...
// Distinct from earliestRecordableDate (input.server.ts): that's a floor on writes, this a floor on reads.
...
// A year, unconditional — unlike masking's policy-dependent lifetime, nothing here needs forgetting.
...
// Precedence URL > cookie > default, written once for both routes. Object.hasOwn, not `in` (a hand-edited "?range=toString" must not match RANGES.toString).
...
// The payload block a loader spreads into its return — both routes assembled this by hand before spec 0015; every field but custom/customMax derives from a type this file already declares.
...
// The window a surface's chart draws, plus the control block a loader spreads into its return (spec 0015). `resolved` stays, since Overview still needs resolved.since/resolved.session.
...
// Rest of the query plus this preset's own "?range=" — a whole search string, not React Router's relative resolution (a `to` starting with "?" replaces the entire query).
...
// Remembers an explicit range choice in the cookie (spec 0008), via middleware not a header on the loader's return, so each loader keeps a plain-object shape.
export function chartRangeMiddleware() {
  // Untyped against react-router's MiddlewareFunction: it and the routes' generated Route.MiddlewareFunction disagree on next's return type.
  return async ({ request }: { request: Request }, next: () => Promise<unknown>): Promise<Response> => {
    const response = (await next()) as Response;
    const requested = readChartRange(request);

    // Not onto a redirect: a cookie on a response that isn't the page is a header nobody reads, and every redirect lands somewhere this middleware also runs.
