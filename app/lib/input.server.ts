...
// Line breaks go the same way statement.ts's optionalCell takes them, and for the same reason: a
// single-line box strips them on the way out (HTML Standard, the input element), so a stored break
// is a value no box can post back. Only a forged submission carries one, and for the account
// number that writes a spelling the upload's mismatch check reads as another account (#312).
